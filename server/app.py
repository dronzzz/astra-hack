...
from processing import get_job_status, processing_jobs, process_video_job
import requests
import tempfile
import json
import subprocess
...
import sieve
import threading
# Import the YouTube upload function
from upload_youtube import handle_video_upload
...
# You might want to store this securely
YOUTUBE_API_KEY = 'AIzaSyCipVUnLSuT8OiUUrKzPbrTB1H61M_YTKc'
...
def generate_short():
    """Process a YouTube URL and generate shorts videos"""
    logger.info("Generate short endpoint called")
    try:
        data = request.get_json()
        youtube_url = data.get('youtube_url')
        aspect_ratio = data.get('aspect_ratio', '9:16')
        words_per_subtitle = int(data.get('words_per_subtitle', 2))
        ...
        thread = threading.Thread(
            ...
            args=(job_id, youtube_url, aspect_ratio,
                  words_per_subtitle, font_size)
        )
        thread.daemon = True
        thread.start()
        ...
        logger.error(f"Error in generate_short: {str(e)}")
        return jsonify({
            "status": "error",
...
def serve_output_file(filename):
    ...
    # Log file details
    logger.info(
        f"Serving file: {file_path}, size: {os.path.getsize(file_path)} bytes")
...
# Download endpoint for legacy compatibility


@app.route('/download/<job_id>', methods=['GET'])
def download_job_output(job_id):
    """Serve the output file for a job"""
    logger.info(f"Download request for job: {job_id}")

    # Get the job status to find videos
    status = get_job_status(job_id)

    # Check if the job exists and has videos
    if status.get("status") == "not_found":
        return jsonify({"error": "Job not found"}), 404

    # Find the first available video from the job
    videos = status.get("videos", [])
    if not videos:
        return jsonify({"error": "No videos available for this job"}), 404

    # Get the first video URL, which should be a path like "/shorts_output/filename.mp4"
    video_url = videos[0].get("url")
    if not video_url or not video_url.startswith("/shorts_output/"):
        return jsonify({"error": "Invalid video URL"}), 500

    # Extract the filename from the URL
    filename = video_url.replace("/shorts_output/", "")
    directory = os.path.abspath("shorts_output")
    file_path = os.path.join(directory, filename)
...
def dub_segment():
    ...
    try:
        ...
        logger.info(
            f"Processing dub request for segment {segment_id}, job {job_id}, language {language}")
        ...
        if not job_status:
            logger.warning(
                f"Job {job_id} not found in memory, attempting to reconstruct")
            ...
            if os.path.exists("shorts_output"):
                video_files = [f for f in os.listdir(
                    "shorts_output") if f.startswith(job_id) and f.endswith(".mp4")]
                logger.info(
                    f"Found {len(video_files)} video files for job {job_id}")
                ...
                for video_file in video_files:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Added reconstructed segment: {segment_id_from_file}")
                    except Exception as e:
                        logger.error(
                            f"Error parsing video filename {video_file}: {e}")

                # Also check if there's exactly the segment ID requested
                for video_file in video_files:
                    if segment_id in video_file:
                        logger.info(
                            f"Found exact match for segment ID in filename: {video_file}")
                        ...
                        if not any(v.get("id") == segment_id for v in job_status["videos"]):
                            job_status["videos"].append(video_info)
                            logger.info(
                                f"Added exact match segment: {segment_id}")

        # Display all available segments in the job for debugging
        if job_status and "videos" in job_status:
            available_segments = [v.get("id")
                                  for v in job_status.get("videos", [])]
        ...
        if not segment:
            ...
            if job_status and job_status.get("videos"):
                ...
                logger.info(
                    f"Using fallback segment: {fallback_segment.get('id')}")
                ...
                if os.path.exists(shorts_dir):
                    ...
                    if matching_files:
                        ...
                        logger.info(
                            f"Found video file in directory: {video_file}")
        ...
        video_basename = os.path.basename(segment.get(
            "url", "").replace("/shorts_output/", ""))
        ...
        audio_temp_path = os.path.join(
            tempfile.gettempdir(), f"dubbed_audio_{temp_audio_id}.mp3")
        ...
        try:
            ...
            preserved_subtitle_path = os.path.join(
            ...
            if os.path.exists(preserved_subtitle_path):
                ...
                if subtitle_file:
                    ...
                    if subtitle_text:
                        ...
                        logger.info(
                            f"Extracted text from subtitle: {text_content[:100]}...")
                        ...
                        logger.info(
                            f"Preserved subtitle text to: {preserved_subtitle_path}")

            # If we still don't have text content, check for 'text' or 'reason' field in segment data
            if not text_content:
                logger.warning(
                    "No subtitle file found. Looking for text in segment data.")
                if 'text' in segment:
                    text_content = segment['text']
                    logger.info(
                        f"Using text from segment data: {text_content[:100]}...")
                elif 'reason' in segment:
                    text_content = segment['reason']
                    logger.info(
                        f"Using reason as fallback text: {text_content[:100]}...")
        ...
        logger.info(f"Text content: {text_content[:200]}..." if len(
        ...
        try:
            ...
            response = requests.post(
                multilingual_url, data=form_data, files=files, timeout=120)
            logger.info(
                f"Dubbing service response status: {response.status_code}")
            ...
            try:
                if os.path.exists(audio_temp_path):
                    ...
                    logger.info(
                        f"Deleted temporary audio file: {audio_temp_path}")
            except Exception as e:
        ...
        dubbed_path = os.path.join(
            "shorts_output", f"{video_basename.split('.')[0]}_{language}_{dubbed_id}.wav")
        ...
        dubbed_video_path = os.path.join(
            "shorts_output", f"{video_basename.split('.')[0]}_{language}_{dubbed_id}.mp4")
        ...
        try:
            subprocess.run(dubbed_cmd, check=True,
        ...
        dubbed_thumbnail_path = os.path.join(
            "shorts_output", f"{video_basename.split('.')[0]}_{language}_{dubbed_id}.jpg")
        original_thumbnail = os.path.join(
            "shorts_output", f"{video_basename.split('.')[0]}.jpg")
        ...
        dubbed_video_info[
            "thumbnailUrl"] = f"/shorts_output/{os.path.basename(dubbed_thumbnail_path)}"
...
def extract_video_id(url):
    match = re.search(
        r"(?:v=|youtu\.be/|/v/|/e/|embed/|shorts/|watch\?v=)([\w-]{11})", url)
...
def get_trending_keywords():
    ...
    try:
        ...
        return ["viral", "breaking news", "new release", "2025"]
...
def analyze_trend(title, description):
    ...
    keyword_score = sum(
        1 for word in trending_keywords if word.lower() in text)
...
def youtube_upload():
    ...
    try:
        data = request.json
        logger.info(f"YouTube upload request received: {data}")
        ...
        segment_id = data.get('segmentId')
        job_id = data.get('jobId')
        title = data.get('title', 'Video Short')
        description = data.get('description', 'Generated with AI')
        privacy_status = data.get('privacyStatus', 'public')

        # Optional parameters
        tags = data.get('tags', ['shorts', 'video', 'ai-generated'])
        category = data.get('categoryId', '22')  # Default to People & Blogs

        if not segment_id or not job_id:
            logger.error(
                f"Missing required parameters: segmentId={segment_id}, jobId={job_id}")
            return jsonify({"error": "Missing required parameters: segmentId and jobId"}), 400

        # Get job status to find the video file
        ...
        # Find the segment
        segment = None
        logger.info(f"Looking for segment {segment_id} in job videos")
        for video in job_status.get("videos", []):
            if video.get("id") == segment_id:
                segment = video
        ...
        # Get the video file path
        video_url = segment.get("url")
        logger.info(f"Video URL from segment: {video_url}")
        ...
        filename = video_url.replace("/shorts_output/", "")
        file_path = os.path.join(os.path.abspath("shorts_output"), filename)
        ...
        # Check if we have client_secrets.json
        client_secrets_file = "client_secrets.json"
        if not os.path.exists(client_secrets_file):
            logger.error("YouTube API credentials not configured on server")
            return jsonify({"error": "YouTube API credentials not configured on server"}), 500

        # Use segment title if available
        video_title = title or segment.get(
            "title", f"Short video {segment_id}")

        # Handle the upload
        logger.info(
            f"Calling handle_video_upload with file_path={file_path}, title={video_title}")
        upload_result = handle_video_upload(
            file_path=file_path,
            title=video_title,
        ...
        return jsonify({
            ...
            "youtubeUrl": upload_result.get("youtube_url")
        })

    except Exception as e:
        error_msg = f"Error uploading to YouTube: {str(e)}"
